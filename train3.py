...
from Models.CycleAug import Generator, Discriminator
...
netG_A_map = Generator(classes, 'bn').build()
netG_B_map = Generator(classes, 'bn').build()
...
city_img = [os.path.join(city_img_root, path.split(os.sep)[-2],
...
for epoch in range(epochs):
    print(epoch)
    # if epoch % 10 == 0:
    #     current_learning_rate = learning_rate * 0.99 ** epoch
