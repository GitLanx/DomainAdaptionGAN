...
netG_A = Generator(3, 'bn').build()
netG_B = Generator(3, 'bn').build()
netD_A = Discriminator(classes, 'bn').build()
netD_B = Discriminator(classes, 'bn').build()
...
for epoch in range(epochs):
    ...
    for ((real_src, real_src_label), (real_target, _)) in dataset:
        with tf.GradientTape() as netG_A_tape, \
            ...
            disc_target_fake, fake_tar_seg = netD_A(fake_target, training=True)
            disc_source_fake, fake_src_seg = netD_B(fake_src, training=True)
            disc_target_real, real_tar_seg = netD_A(real_target, training=True)
            disc_source_real, real_src_seg = netD_B(real_src, training=True)
            ...
            loss_seg_B = categorical_loss(real_src_label, real_src_seg, 5)
            ...
            loss_D_A = (loss_D_A_real + loss_D_A_fake) * 0.5 + loss_seg_A
            ...
            loss_D_B = (loss_D_B_real + loss_D_B_fake) * 0.5 + loss_seg_B
...
netG_A.save_weights("./bn_ckpt200/")
